...
/**
 * The line being spoken at `ms`: the last one that has started, so a pause between two
 * lines keeps the one just said. Nothing before the first line starts.
 */
export function segmentIndexAt(segments: readonly TranscriptSegment[], ms: number): number {
  let low = 0
